...
/*   Updated: 2024/02/19 13:42:04 by tfrily           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/**
 * @brief  Check that only correct elements are on the 
 * 			map and the map is not empty or file start with \n 
 * 
...
/**
 * @brief Check if the map is possible using floodfill algorithme. 
 * 		Begin at the player position.
 * 
...
void	ft_chk_map_validity(int p_position[2], t_data *data)
{
	...
	if (ft_count_elements(data->map->map, 'C') != 0
		|| ft_count_elements(data->map->map, 'E') != 0)
	{
